...
_PRIORITY_COLOR = {"high": "#c2410c", "medium": "#d97706", "low": _MUTED}


def _skills_block(skills: Dict) -> str:
    """'Skills to acquire' section, appended on the weekly run."""
    gaps = (skills or {}).get("gaps") or []
    if not gaps:
        return ""
    rows = ""
    for i, g in enumerate(gaps, 1):
        pri = (g.get("priority") or "medium").lower()
        kw = ", ".join(g.get("keywords") or [])
        rows += (
            f'<table role="presentation" cellpadding="0" cellspacing="0" bgcolor="{_CARD}" '
            f'style="width:100%;background:{_CARD};border:1px solid {_BORDER};border-radius:12px;'
            f'margin:0 0 12px;border-collapse:separate"><tr><td style="padding:14px 16px">'
            f'{_pill(pri, _PRIORITY_COLOR.get(pri, _MUTED), filled=True)}'
            f'<div style="font:600 16px/1.35 {_FONT};color:{_TEXT};margin-top:8px">'
            f'{i}. {_esc(g.get("skill") or "")}</div>'
            + (_label("Why now") + _para(g["why"]) if g.get("why") else "")
            + ((_label("Learn / search keywords")
                + f'<div style="font:600 12.5px {_MONO};color:{_PRIMARY};line-height:1.5">{_esc(kw)}</div>')
               if kw else "")
            + (_label("Seen in") + _para(g["evidence"], color=_MUTED) if g.get("evidence") else "")
            + '</td></tr></table>'
        )
    summary = (skills or {}).get("summary") or ""
    return (
        f'<div style="margin-top:28px;padding-top:20px;border-top:2px solid {_PRIMARY}">'
        f'<div style="font:700 10px {_MONO};letter-spacing:.2em;text-transform:uppercase;'
        f'color:{_PRIMARY}">Skills gap analysis</div>'
        f'<div style="font:700 22px/1.2 {_FONT};color:{_TEXT};margin-top:8px">Skills to acquire</div>'
        + (f'<div style="font:400 13.5px/1.55 {_FONT};color:{_MUTED};margin:8px 0 4px">{_esc(summary)}</div>'
           if summary else "")
        + f'<div style="font:500 11px {_MONO};color:{_MUTED};margin:8px 0 16px;letter-spacing:.04em">'
          f'{skills.get("articles_considered", 0)} ITEMS REVIEWED · '
          f'{skills.get("skills_known", 0)} SKILLS ON FILE</div>'
        + rows
        + f'<div style="font:400 11.5px/1.6 {_FONT};color:{_MUTED};margin-top:4px">'
          f'Learned one? Run <span style="font-family:{_MONO}">./skills.py learned "&lt;skill&gt;"</span> '
          f'so it stops appearing here.</div></div>'
    )


def render_news_html(news: Dict, *, title: str = "AI News Briefing",
                     detail_limit: int = 20, skills: Dict | None = None) -> str:
    ...
    footer = (f'<div style="font:400 11px {_FONT};color:{_MUTED};text-align:center;'
              ...
              f'AI News Briefing · generated automatically</div>')
